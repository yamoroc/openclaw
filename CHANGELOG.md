...
- Config/Plugins validation backward compatibility: unknown plugin IDs in `plugins.allow`, `plugins.deny`, or `plugins.slots.memory` now emit warnings instead of failing validation, and are sanitized at runtime so local plugin customizations are not ignored after upgrades. (#11)
